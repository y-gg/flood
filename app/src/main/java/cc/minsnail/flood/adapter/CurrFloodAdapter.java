 ...
 */
public class CurrFloodAdapter extends BaseAdapter {
    private Context context;
    private List<FloodSum> items;
    public CurrFloodAdapter(Context context){
        this.context = context;
        items = new ArrayList<>();
    }
    public void addItem(FloodSum item){
        if (item!=null){
            items.add(item);
        }
        notifyDataSetChanged();
    }
    public void addAll(List<FloodSum> items){
        for (FloodSum item:items){
            if (item!=null){
                this.items.add(item);
            }
        }
